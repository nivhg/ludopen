...
void F_HistoriqueJeux::AfficherHistorique( unsigned int nIdMembre )
{
    ...
    if( queryHistorique.exec() )
    {
        ...
        while( queryHistorique.next() )
        {
            ...
            ModeleHistoriqueJeuxEmprunter.setItem( i, 1, new QStandardItem( queryHistorique.record().value( 1 ).toDateTime().toString( "dd.MM.yy" ) ) ) ;
            ModeleHistoriqueJeuxEmprunter.setItem( i, 2, new QStandardItem( queryHistorique.record().value( 2 ).toDateTime().toString( "dd.MM.yy" ) ) ) ;
